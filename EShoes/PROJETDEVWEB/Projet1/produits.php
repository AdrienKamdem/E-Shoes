...
<?php
 if (isset($_GET['categorie'])) 
 {
  ...
  if(in_array($_GET['categorie'],array('sneakers', 'chaussures de ville', 'chaussures de sport')))
  { $id=0;
                  ...
                  <div class="commander">
                    ...
                    <div class="ajouterPanier">
                      <?php if (!isset($_SESSION['email'])) {
                       ?>
                      <a href="index.php" onclick="alert('Veuillez vous connecter pour acceder à un panier');"><button class="btn-panier btn-add">Ajouter au panier</button></a>
                       <?php
                      } else { ?>
                     <a href="#" onclick=" this.href='panier.php?action=add&amp;quantite='+ document.querySelector('#quantite<?= $key; ?>').value +'&amp;idproduit=<?= $id?>&amp;cle=<?= $cat?>'"><button class="btn-panier btn-add" id='add<?= $key ?>'>Ajouter au panier</button></a> 
                     <?php } ?>
                    </div>
                </div>
